...
...............\`)`);
  ...
  it("should score blank tiles as zero", () => {
    let game = new Game();
    // no multipliers
    expect(game.playTiles(0, 0, "r", "zOO", true)).to.equal(2);
    // triple word
    expect(game.playTiles(0, 0, "r", "zOOS", true)).to.equal(9);
    // triple letter on Z
    expect(game.playTiles(8, 0, "r", "zOO", true)).to.equal(2);
    // triple word and triple letter on Z
    expect(game.playTiles(8, 0, "r", "zOOS", true)).to.equal(9);
  });
...
...............\`)`);
  ...
  it("should differentiate between no perpendicular words and zero scoring perpendicular words", () => {
    let game = new Game();
    expect(game.playTiles(1, 0, "d", "p")).to.equal(0);
    expect(
      game.playTiles(0, 1, "r", "Pi", undefined, undefined, true)
    ).to.equal(3);
